...
this.planets = ["Sun", "Mercury", "Venus", "Earth", "Mars", "Jupiter", "Saturn", "Uranus", "Neptune", "Moon"];

let planetsMesh = [];

let stateValuesPlanets = [0,0,0,0,0,0,0,0,0];
...
function initEvents() {
   ...
   /* document.addEventListener( 'mousedown', function(event) {
        onMouseClick(event,scene.children)
	}, false );*/
}
// Loop runner
...
function setStatePlanet(planet) {
...
function incrementeValuePlanets(indexState) {

	let incrementValue = stateValuesPlanets[indexState] + 1;
    stateValuesPlanets[indexState] = incrementValue;
	if(incrementValue === 2) {
		return true;
	}
}
...
function setFocusPlanet(planet){
	...
	let planetValue = setStatePlanet(planet);
	if(incrementeValuePlanets(planetValue)) {
        console.log("2 fois sur : " );
        let newUrl = "localhost:8888http://localhost:8888/Alpha_leonis/src/php/info.php?planete=" + planet;
        console.log(newUrl);
...
var Planet = function(planet, domElement){
	var planetSceneName;
    ...
    planetsMesh.push(this.planetMesh);
	...
	this.controlsPlanet = new THREE.OrbitControls( this.cameraPlanet, domElement);
...
for(var i=0; i < this.planets.length; i++){
	...
	window[this.planets[i]] = new Planet(newPlanet, domElement);
}
